...
function App() {
  ...
  <div className="max-w-7xl mx-auto">
    ...
    <label className="mt-6 text-center text-3xl font-extrabold text-gray-900 mb-6">
      My Guess:
      <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={guess}  onChange={e => setGuess(e.target.value)}/>
    </label>
    ...
    <List>
    {cast.map((actor) => {
      return <ListItem actor={actor}></ListItem>
    })}
    </List>
    </div>
    </div>
